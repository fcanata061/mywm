...
# Gerenciamento de atalhos e teclas para MyWM 1.0+
...
class KeyBindings:
    def __init__(self, wm, config):
        ...
        config: dict externo com atalhos
        """
        self.wm = wm
    ...
    def reload_config(self):
        ...
        if hasattr(self.wm, "keybindings"):
            self._setup_default_bindings()
    ...
    def grab_keys(self):
        ...
        for combo in self.bindings:
            # Conversão simplificada: Mod4+KeySym
            parts = combo.split("+")
            mod = 0
            key = parts[-1]
            if "Mod4" in parts:
    ...
    # =======================
    # EXECUTAR AÇÃO DE TECLA
    # =======================
    def handle_key_press(self, event):
        keysym = self.wm.dpy.keycode_to_keysym(event.detail, 0)
        # Converte keysym para string simplificada
        key = XK.keysym_to_string(keysym)
        # Itera sobre bindings para executar ação
